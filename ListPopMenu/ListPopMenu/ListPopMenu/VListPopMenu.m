...
@interface VListPopMenuCell : UITableViewCell
@property (nonatomic, assign) BOOL isShowSeparator;
@property (nonatomic, strong) UIColor * separatorColor;
@end

@implementation VListPopMenuCell
...
@interface VListPopMenu () <UITableViewDelegate, UITableViewDataSource>
...
@property (nonatomic, strong) UIColor     * separatorColor; //菜单间分割线颜色
@property (nonatomic) CGRect                relyRect;       //按钮响应视图大小
...
#pragma mark - publics
+ (VListPopMenu *)showAtPoint:(CGPoint)point titles:(NSArray *)titles icons:(NSArray *)icons menuWidth:(CGFloat)itemWidth delegate:(id<VListPopMenuDelegate>)delegate {
    VListPopMenu *popupMenu = [[VListPopMenu alloc] init];
...
+ (VListPopMenu *)showRelyOnView:(UIView *)view titles:(NSArray *)titles icons:(NSArray *)icons menuWidth:(CGFloat)itemWidth delegate:(id<VListPopMenuDelegate>)delegate {
...
+ (VListPopMenu *)showAtPoint:(CGPoint)point titles:(NSArray *)titles icons:(NSArray *)icons menuWidth:(CGFloat)itemWidth otherSettings:(void (^) (VListPopMenu * popupMenu))otherSetting {
    VListPopMenu *popupMenu = [[VListPopMenu alloc] init];
    popupMenu.point = point;
    popupMenu.titles = titles;
    popupMenu.images = icons;
...
+ (VListPopMenu *)showRelyOnView:(UIView *)view titles:(NSArray *)titles icons:(NSArray *)icons menuWidth:(CGFloat)itemWidth otherSettings:(void (^) (VListPopMenu * popupMenu))otherSetting {
    CGRect absoluteRect = [view convertRect:view.bounds toView:kKeyWindow];
    CGPoint relyPoint = CGPointMake(absoluteRect.origin.x + absoluteRect.size.width / 2, absoluteRect.origin.y + absoluteRect.size.height);
    VListPopMenu *popupMenu = [[VListPopMenu alloc] init];
    popupMenu.point = relyPoint;
...
- (void)dismiss {
    if (self.delegate && [self.delegate respondsToSelector:@selector(listPopMenuBeganDismiss)]) {
        [self.delegate listPopMenuBeganDismiss];
    }
    [UIView animateWithDuration: 0.25 animations:^{
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(listPopMenuDidDismiss)]) {
            [self.delegate listPopMenuDidDismiss];
        }
        self.delegate = nil;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    static NSString * identifier = @"VListPopMenu";
    VListPopMenuCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (!cell) {
        cell = [[VListPopMenuCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:identifier];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(listPopMenuDidSelectedAtIndex:VListPopMenu:)]) {
        
        [self.delegate listPopMenuDidSelectedAtIndex:indexPath.row VListPopMenu:self];
    }
}

#pragma mark - scrollViewDelegate
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView {
    VListPopMenuCell *cell = [self getLastVisibleCell];
...
- (VListPopMenuCell *)getLastVisibleCell {
...
- (void)show {
    [kKeyWindow addSubview:_menuBackView];
    [kKeyWindow addSubview:self];
    VListPopMenuCell *cell = [self getLastVisibleCell];
    cell.isShowSeparator = NO;
    if (self.delegate && [self.delegate respondsToSelector:@selector(listPopMenuBeganShow)]) {
        [self.delegate listPopMenuBeganShow];
    }
    self.layer.affineTransform = CGAffineTransformMakeScale(0.1, 0.1);
    [UIView animateWithDuration: 0.25 animations:^{
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(listPopMenuDidShow)]) {
            [self.delegate listPopMenuDidShow];
        }
    }];
}
...
- (void)setDefaultSettings {
    ...
    _type = VListPopMenuTypeDefault;
    _arrowDirection = VListPopMenuArrowDirectionTop;
    _priorityDirection = VListPopMenuPriorityDirectionTop;
    _minSpace = 10.0;
    _maxVisibleCount = 5;
    _itemHeight = 44;
    _isCornerChanged = NO;
    ...
    _menuBackView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kScreenSize.width, kScreenSize.height)];
...
- (void)setType:(VListPopMenuType)type {
    _type = type;
    switch (type) {
        case VListPopMenuTypeDark:
        {
...
- (void)setArrowDirection:(VListPopMenuArrowDirection)arrowDirection {
...
- (void)setPriorityDirection:(VListPopMenuPriorityDirection)priorityDirection {
...
- (void)updateUI {
    ...
    if (_priorityDirection == VListPopMenuPriorityDirectionTop) {
        if (_point.y + height + _arrowHeight > kScreenSize.height - _minSpace) {
            _arrowDirection = VListPopMenuArrowDirectionBottom;
            _isChangeDirection = YES;
        }else {
            _arrowDirection = VListPopMenuArrowDirectionTop;
            _isChangeDirection = NO;
        }
    }else if (_priorityDirection == VListPopMenuPriorityDirectionBottom) {
    ...
    }else if (_priorityDirection == VListPopMenuPriorityDirectionLeft) {
        if (_point.x + _itemWidth + _arrowHeight > kScreenSize.width - _minSpace) {
            _arrowDirection = VListPopMenuArrowDirectionRight;
            _isChangeDirection = YES;
        }else {
            _arrowDirection = VListPopMenuArrowDirectionLeft;
            _isChangeDirection = NO;
        }
    }else if (_priorityDirection == VListPopMenuPriorityDirectionRight) {
    ...
    if (_arrowDirection == VListPopMenuArrowDirectionTop) {
        CGFloat y = _isChangeDirection ? _point.y  : _point.y;
        if (_arrowPosition > _itemWidth / 2) {
            self.frame = CGRectMake(kScreenSize.width - _minSpace - _itemWidth, y, _itemWidth, height + _arrowHeight);
    ...
    }else if (_arrowDirection == VListPopMenuArrowDirectionBottom) {
    ...
    }else if (_arrowDirection == VListPopMenuArrowDirectionLeft) {
    ...
    }else if (_arrowDirection == VListPopMenuArrowDirectionRight) {
    ...
    }else if (_arrowDirection == VListPopMenuArrowDirectionNone) {
...
- (void)changeRectCorner {
    ...
    if (_arrowDirection == VListPopMenuArrowDirectionTop || _arrowDirection == VListPopMenuArrowDirectionBottom) {
    ...
    }else if (_arrowDirection == VListPopMenuArrowDirectionLeft || _arrowDirection == VListPopMenuArrowDirectionRight) {
        if (haveTopLeftCorner) {
            _rectCorner = _rectCorner | UIRectCornerTopRight;
        }else {
            _rectCorner = _rectCorner & (~UIRectCornerTopRight);
...
- (void)setArrowPosition {
    if (_priorityDirection == VListPopMenuPriorityDirectionNone) {
        return;
    }
    
    if (_arrowDirection == VListPopMenuArrowDirectionTop || _arrowDirection == VListPopMenuArrowDirectionBottom) {
        if (_point.x + _itemWidth / 2 > kScreenSize.width - _minSpace) {
            _arrowPosition = _itemWidth - (kScreenSize.width - _minSpace - _point.x);
        ...
        //        if (_point.y + _itemHeight / 2 > kScreenSize.height - _minSpace) {
        //            _arrowPosition = _itemHeight - (kScreenSize.height - _minSpace - _point.y);
...
- (void)drawRect:(CGRect)rect {
    UIBezierPath *bezierPath = [VListPopMenuPath bezierPathWithRect:rect rectCorner:_rectCorner cornerRadius:_cornerRadius borderWidth:_borderWidth borderColor:_borderColor backgroundColor:_backColor arrowWidth:_arrowWidth arrowHeight:_arrowHeight arrowPosition:_arrowPosition arrowDirection:_arrowDirection];
    [bezierPath fill];
    [bezierPath stroke];
}
